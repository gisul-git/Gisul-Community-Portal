...
export default function UnifiedTrainerSearch({ token }) {
  ...
  const buildJDTextFromForm = () => {
    ...
    if (experienceYears)
      text += `Experience Required: ${experienceYears} years\n`;
  ...
  async function handleJDSearch() {
    ...
    if (!jdTextToUse.trim()) {
      alert(
        "Please upload a JD file or fill in the JD form (Domain and Skills are required)",
      );
      return;
    }
  ...
  const handleDownloadPDF = async (identifier, name) => {
    try {
      if (!identifier) {
        alert(
          "Cannot download PDF: Trainer identifier (email or profile ID) is missing",
        );
        return;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.detail || `Failed to download PDF (${response.status})`,
        );
      }
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/admin/export_trainers_to_excel`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="bg-[#f8fafc] min-h-screen p-4 sm:p-6 md:p-10 font-sans">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 mb-12">
          ...
          <div className="xl:col-span-5">
            <div className="bg-white rounded-[40px] p-8 md:p-10 shadow-[0_20px_50px_rgba(105,83,163,0.03)] border border-purple-50 h-full flex flex-col relative overflow-hidden group">
              ...
              <div className="relative mb-8 group/upload">
                ...
                />
                <div
                  className={`border-4 border-dashed rounded-[32px] p-10 text-center transition-all duration-500 ${
                    uploadedFileText
                      ? "border-green-400 bg-green-50/30"
                      : "border-purple-100 bg-purple-50/20 group-hover/upload:border-purple-300"
                  }`}
                >
                  <div
                    className={`w-16 h-16 rounded-[24px] mx-auto mb-4 flex items-center justify-center transition-all ${uploadedFileText ? "bg-green-100" : "bg-purple-100"}`}
                  >
                    <svg
                      className="w-8 h-8"
                      style={{
                        color: uploadedFileText ? "#10b981" : "#6953a3",
                      }}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                  ...
                  <p className="text-lg font-bold text-gray-800 mb-1">
                    {uploadedFileText ? "JD Attached" : "Drop JD Here"}
                  </p>
              ...
              <button
                ...
                disabled={
                  jdLoading ||
                  uploading ||
                  (!uploadedFileText && !buildJDTextFromForm().trim())
                }
              ...
              {parsedJD && (
                <div className="mt-8 p-6 rounded-[28px] bg-gradient-to-br from-blue-50/50 to-purple-50/50 border border-blue-100/50">
                  <p className="text-[10px] font-black text-[#6953a3] uppercase tracking-widest mb-3">
                    AI Intelligence Extraction
                  </p>
                  {parsedJD.skills && (
                    <p className="text-xs font-bold text-gray-700 leading-relaxed mb-1">
                      <span className="opacity-50 uppercase mr-2">Skills:</span>{" "}
                      {parsedJD.skills.join(", ")}
                    </p>
                  )}
                  {parsedJD.experience_years && (
                    <p className="text-xs font-bold text-gray-700">
                      <span className="opacity-50 uppercase mr-2">Tenure:</span>{" "}
                      {parsedJD.experience_years} Years
                    </p>
                  )}
                </div>
              )}
          ...
          <div className="xl:col-span-7">
            <div className="h-full bg-white rounded-[40px] p-12 md:p-20 shadow-[0_20px_50px_rgba(105,83,163,0.03)] border border-purple-50 flex flex-col justify-center items-center text-center relative overflow-hidden">
              ...
              {/* Reduced container width from max-w-2xl to max-w-xl */}
              <div className="max-w-xl w-full z-10">
                <div className="w-20 h-20 bg-[#f4e403]/10 rounded-[28px] flex items-center justify-center mx-auto mb-8 transform rotate-12 shadow-xl shadow-yellow-50/50">
                  <svg
                ...
                <h2 className="text-5xl font-black text-gray-900 mb-3 tracking-tighter uppercase">
                  Trainer Search
                </h2>
                <p className="text-gray-400 font-bold text-lg mb-8 leading-relaxed">
                  Discover experts using semantic natural language context.
                </p>

                <div className="relative group mb-8">
                  <input
                    type="text"
                    placeholder="Search e.g. 'React trainer'..."
                    className="w-full pl-8 pr-36 py-5 rounded-[30px] bg-gray-50 border-2 border-transparent focus:bg-white focus:border-purple-100 transition-all outline-none text-lg font-bold shadow-inner placeholder-gray-300"
                    value={searchQuery}
                  ...
                  <button
                    ...
                    className="absolute right-2.5 top-2.5 bottom-2.5 px-8 rounded-[24px] text-white font-black text-base transition-all active:scale-[0.95] shadow-lg shadow-purple-100"
                    style={{ backgroundColor: "#6953a3" }}
                  >
                    {textLoading ? "..." : "Search"}
                  </button>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                  {["SAP HANA", "NodeJS Expert", "Soft Skills", "DevOps"].map(
                    (tag) => (
                      <button
                        key={tag}
                        onClick={() => setSearchQuery(tag)}
                        className="px-4 py-2 bg-gray-50 text-gray-500 rounded-full text-[10px] font-black hover:bg-[#6953a3] hover:text-white transition-all border border-gray-100 uppercase tracking-widest shadow-sm"
                      >
                        {tag}
                      </button>
                    ),
          ...
          <div ref={resultsRef} className="sticky top-6 z-50 mb-12">
            <div className="bg-white/95 backdrop-blur-2xl px-12 py-8 rounded-[48px] shadow-[0_30px_90px_rgba(0,0,0,0.12)] border border-white/50 flex flex-col md:flex-row justify-between items-center gap-8">
              <div className="flex items-center gap-6">
                {/* Dynamic Icon/Loader Container */}
                <div
                  className={`w-16 h-16 rounded-3xl flex items-center justify-center shadow-inner relative overflow-hidden transition-colors duration-500 ${isLoading ? "bg-purple-100" : "bg-purple-50"}`}
                >
                  {isLoading ? (
                    // Animated Loader SVG
                    <svg
                      className="w-8 h-8 text-[#6953a3] animate-spin"
                      fill="none"
                      viewBox="0 0 24 24"
                    >
                      <circle
                    ...
                    // Original Expert Icon
                    <svg
                      className="w-8 h-8 text-[#6953a3] relative z-10"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <div>
                  {isLoading ? (
                    <div className="flex flex-col gap-2">
                      <p className="font-black text-[#6953a3] text-3xl leading-none uppercase tracking-tighter animate-pulse">
                        Searching Data...
                      </p>
                      <p className="text-[10px] font-black text-purple-300 uppercase tracking-[0.4em]">
                        Please wait
                      </p>
                    </div>
                  ) : (
                    <>
                      <p className="font-black text-gray-900 text-3xl leading-none mb-1 uppercase tracking-tighter">
                        Expert Discovery
                      </p>
                      <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">
                        Optimized Cluster Ranking
                      </p>
                    </>
                  )}
                </div>
              ...
              {/* Action Buttons: Only visible when NOT loading */}
              {!isLoading && (
                <div className="flex items-center gap-5">
                  <div className="flex items-center gap-4 bg-gray-50 px-8 py-5 rounded-3xl border border-gray-100 shadow-inner group/select">
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked)
                          currentResults.forEach((r) =>
                            allIds.add(r.email || r.profile_id),
                          );
                        else
                          currentResults.forEach((r) =>
                            allIds.delete(r.email || r.profile_id),
                          );
                    ...
                    <label
                      ...
                      className="text-xs font-black text-gray-400 uppercase tracking-widest cursor-pointer select-none group-hover/select:text-gray-600 transition-colors"
                    >
                      Global Select
                    </label>
                  </div>
                  ...
                  {selectedTrainers.size > 0 && (
                    <div className="flex items-center gap-5 animate-in fade-in slide-in-from-right-10 duration-700">
                      <div
                        className="px-10 py-5 rounded-[24px] font-black text-sm shadow-2xl shadow-yellow-100/50"
                        style={{ backgroundColor: "#f4e403" }}
                      >
                  ...
                  <button
                    onClick={handleClear}
                    className="p-5 text-gray-200 hover:text-red-500 transition-all hover:rotate-90"
                  >
                    <svg
                      className="w-8 h-8"
                      fill="none"
        ...
        {!isLoading &&
          ...
          activeSearchType === "text" && (
            <div className="text-center py-40 bg-white rounded-[64px] border-8 border-dashed border-gray-50/50 shadow-inner">
              ...
              <p className="text-4xl font-black text-gray-300 tracking-tighter mb-4 uppercase">
                No results found
              </p>
              <p className="text-gray-400 font-bold text-sm uppercase tracking-[0.4em] mb-12">
                Search query yielded no matches.
              </p>
              <button
                onClick={handleClear}
                className="px-14 py-6 bg-[#6953a3] text-white font-black rounded-3xl text-xs tracking-[0.4em] shadow-2xl shadow-purple-200 transition-all active:scale-95"
              >
                Search Again
              </button>
            </div>
        ...
        {!isLoading &&
          ...
          activeSearchType === "jd" && (
            <div className="text-center py-40 bg-white rounded-[64px] border-8 border-dashed border-gray-50/50 shadow-inner">
              <div className="w-28 h-28 bg-gray-50 rounded-[48px] flex items-center justify-center mx-auto mb-10 shadow-inner">
                <svg
                  className="w-14 h-14 text-gray-200/40"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <p className="text-gray-400 font-bold text-sm uppercase tracking-[0.4em]">
                The JD parameters do not correlate with existing trainer nodes.
              </p>
            </div>
          )}
        ...
        {!isLoading &&
          ...
          currentResults.length === 0 && (
            <div className="text-center py-20 border-4 border-dashed border-purple-100 rounded-[56px] bg-[#6953a3]/[0.01]">
              <p className="text-gray-300 font-black uppercase text-xs tracking-[0.4em] px-10 leading-loose opacity-60">
                Initialize Neural Discovery or analyze a JD to view matches
              </p>
            </div>
          )}
        ...
        {!isLoading && currentResults.length > 0 && (
          // CHANGED: xl:grid-cols-3 to xl:grid-cols-4 and reduced gap slightly for better fit
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 pb-32">
            {currentResults.map((trainer, i) => {
              ...
              const score =
                trainer.match_percentage !== undefined
                  ? trainer.match_percentage
                  : Math.round(
                      (trainer.score || trainer.match_score || 0) * 100,
                    );
              ...
              return (
                <div
                  ...
                  // CHANGED: Reduced padding from p-10 to p-8 to fit 4 cards nicely
                  className={`group bg-white rounded-[48px] p-8 border-4 transition-all duration-500 cursor-pointer relative flex flex-col ${
                    isSelected
                ...
                >
                  <div
                    // CHANGED: Adjusted absolute positioning and padding for the top label
                    className="absolute top-0 right-0 px-6 py-4 rounded-bl-[32px] rounded-tr-[44px] font-black text-[10px] text-white z-20 shadow-xl"
                    style={{ backgroundColor: "#6953a3" }}
                  >
                    {score}% MATCH
                  </div>

                  {/* CHANGED: Adjusted avatar size and margin */}
                  <div className="flex items-center gap-5 mb-8">
                    <div className="w-16 h-16 rounded-[24px] bg-purple-50 flex items-center justify-center text-[#6953a3] font-black text-2xl shadow-inner shrink-0 group-hover:scale-110 transition-transform duration-500">
                      {trainer.name?.[0] || "T"}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="font-black text-gray-900 text-2xl leading-none mb-2 truncate tracking-tighter uppercase">
                        {trainer.name || "Anonymous"}
                      </h4>
                      <p className="text-gray-400 text-[9px] font-black tracking-[0.2em] flex items-center gap-2 uppercase truncate">
                        <svg
                        ...
                        <span className="truncate">
                          {trainer.location && trainer.location.trim()
                            ? trainer.location
                            : "Global Remote"}
                        </span>
                      </p>
                  ...
                  <div className="space-y-6 flex-1">
                    {trainer.email && (
                      <div className="text-xs text-gray-600 mb-2 break-all leading-relaxed">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-3">
                        ...
                        {trainer.skills &&
                          trainer.skills.join(", ").length > 80 && ( // Lowered threshold since card is narrower
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                toggleSkills(i);
                              }}
                              className="text-[9px] font-black text-[#6953a3] underline underline-offset-4 tracking-widest uppercase"
                            >
                              {expandedSkills[i] ? "MIN" : "EXPAND"}
                            </button>
                          )}
                      </div>
                      <div className="flex flex-wrap gap-2">
                        {trainer.skills && trainer.skills.length > 0 ? (
                          (expandedSkills[i]
                            ? trainer.skills
                            : trainer.skills.slice(0, 4) // Show fewer default skills
                          ).map((skill, idx) => (
                            <span
                              key={idx}
                              className="px-3 py-1.5 bg-gray-50 text-gray-500 rounded-xl text-[10px] font-black border border-gray-100 group-hover:bg-white group-hover:border-purple-200 transition-all uppercase tracking-wide truncate max-w-full"
                            >
                              {skill}
                            </span>
                          ))
                        ) : (
                          <span className="text-[10px] text-gray-300 font-bold italic tracking-wider">
                            No Matrix Found
                          </span>
                        )}
                        {!expandedSkills[i] && trainer.skills?.length > 4 && (
                          <span className="text-[10px] font-black text-[#6953a3] py-1.5 opacity-60">
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-6 border-t border-gray-50">
                      <div className="bg-gray-50/70 rounded-[24px] p-4 group-hover:bg-white transition-colors border border-gray-100/50 flex flex-col items-center justify-center">
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1 text-center">
                          Tenure
                        </p>
                      ...
                      <div className="bg-gray-50/70 rounded-[24px] p-4 group-hover:bg-white transition-colors border border-gray-100/50 flex flex-col items-center justify-center overflow-hidden">
                        <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1 text-center">
                          Unit
                        </p>
                        <p className="text-sm font-black text-gray-800 truncate tracking-tight w-full text-center">
                          {trainer.current_company || "N/A"}
                        </p>
                      </div>
                    </div>
                    ...
                    {trainer.education && (
                      <div className="px-1 pt-2">
                        <p className="text-[9px] text-gray-300 font-black uppercase tracking-[0.2em] mb-2">
                          Academic
                        </p>
                        <div className="text-[10px] font-bold text-gray-500 italic leading-relaxed line-clamp-2 opacity-80">
                          {/* Education formatting logic remains the same, just changed font size */}
                          {typeof trainer.education === "string" ? (
                            ...
                            <>
                              {trainer.education.degree && (
                                <span className="font-bold">
                                  {trainer.education.degree}
                                </span>
                              )}
                              {trainer.education.institution && (
                  ...
                  <div className="mt-8 flex gap-3 relative z-10 items-center">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDownloadPDF(identifier, trainer.name);
                      }}
                      className="flex-1 py-4 rounded-[20px] bg-red-50 text-red-500 text-[9px] font-black tracking-[0.2em] transition-all hover:bg-red-500 hover:text-white shadow-md shadow-red-50 hover:shadow-none uppercase"
                    >
                      DL CV
                    </button>

                    <div
                      className={`w-12 h-12 rounded-[20px] border-4 flex items-center justify-center shrink-0 transition-all ${
                        isSelected
                          ? "bg-[#6953a3] border-[#6953a3] text-white shadow-lg scale-110"
                          : "border-gray-50 text-gray-200 group-hover:border-purple-200 group-hover:text-purple-300"
                      }`}
                    >
                      <svg
                        className="w-6 h-6"
