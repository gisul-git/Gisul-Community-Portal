    ...
    try {
      if (!userType) {
        ...
        if (adminRes.access_token) {
          ...
          setError("Please select 'Trainer' or 'Client'. Admin credentials not recognized.");
          setLoading(false);
          return;
        }
      }
        ...
        <div className="hidden lg:flex flex-col w-1/2 justify-center space-y-8 h-full">
          ...
          <div className="space-y-4">
            <h2 className="text-6xl font-black text-slate-800 leading-[1.1] tracking-tight padding-right-4">
              Gisul <br /> 
              {/* Updated Gradient to include Yellow/Gold */}
            ...
            <p className="text-lg text-slate-500 max-w-md leading-relaxed font-medium">
              The professional hub for collaboration. Connect with Trainers and Clients in one unified ecosystem.
            </p>
          </div>
        ...
        <div className="w-full max-w-[440px] flex flex-col justify-center h-full py-6">
          ...
          <div className="bg-white/90 backdrop-blur-xl border border-white/60 p-8 md:p-10 rounded-3xl shadow-[0_8px_40px_-12px_rgba(0,0,0,0.1)] flex flex-col justify-center relative overflow-hidden">
            ...
            <div className="mb-8">
              ...
              <p className="text-slate-500 mt-1 text-sm font-medium">Authentication required to proceed.</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {[
                ...
                >
                  ...
                  {userType === role.id && <span className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse"></span>}
                  
                  <span className="text-lg">{role.icon}</span>
                  <span className={`text-xs font-bold uppercase tracking-wider ${userType === role.id ? "text-slate-800" : "text-slate-400"}`}>
                    {role.label}
                  </span>
                </button>
              ))}
            ...
            <div className="relative my-8">
              ...
              <div className="relative flex justify-center text-[10px] uppercase tracking-widest font-bold text-slate-400"><span className="bg-[#fffcfc] px-3">Alternative Access</span></div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <button 
                type="button"
                onClick={() => userType && (window.location.href = `${API_BASE}/api/auth/google/login/${userType === "trainer" ? "trainer" : "customer"}`)}
                disabled={!userType}
              ...
              <button 
                type="button"
                onClick={() => userType && (window.location.href = `${API_BASE}/api/auth/microsoft/login/${userType === "trainer" ? "trainer" : "customer"}`)}
                disabled={!userType}
                className="flex items-center justify-center py-2.5 border border-slate-200 rounded-lg hover:bg-slate-50 transition-all disabled:opacity-50 bg-white"
              >
            ...
            <p className="text-center mt-8 text-slate-400 text-xs">
              Not part of the community? <button onClick={onSwitchToSignup} className="text-amber-600 font-bold hover:underline hover:text-amber-700 transition-colors">Sign up</button>
            </p>
          </div>
        </div>
      </div>
